...
export function HistoryPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-8">
      <div className="mb-6">
        <h1 className="text-2xl font-black text-ink tracking-tight mb-1">History</h1>
        <p className="text-sm text-muted">{total} session{total !== 1 ? "s" : ""}</p>
      </div>

      <div className="flex flex-wrap gap-3 mb-6">
        <div className="grid gap-1">
          <label className="text-[10px] font-extrabold text-muted uppercase">Game</label>
          <select
            value={game}
            onChange={(e) => applyFilter("game", e.target.value)}
            className="min-h-[38px] text-ink bg-surface-container border rounded-input px-3 text-sm outline-none hover:border-accent/40 focus:border-accent focus:shadow-[0_0_0_3px_var(--color-accent-soft)] border-line/40"
          >
        ...
        <div className="grid gap-1">
          <label className="text-[10px] font-extrabold text-muted uppercase">Agent</label>
          <input
            type="text"
            value={agent}
            onChange={(e) => applyFilter("agent", e.target.value)}
            placeholder="Filter by agent..."
            className="min-h-[38px] text-ink bg-surface-container border rounded-input px-3 text-sm outline-none hover:border-accent/40 focus:border-accent focus:shadow-[0_0_0_3px_var(--color-accent-soft)] border-line/40 w-48"
          />
        </div>

        <div className="grid gap-1">
          <label className="text-[10px] font-extrabold text-muted uppercase">From</label>
          <input
            type="date"
            value={dateFrom}
        ...
        <div className="grid gap-1">
          <label className="text-[10px] font-extrabold text-muted uppercase">To</label>
          <input
            type="date"
            value={dateTo}
      ...
      {loading ? (
        ...
        <div className="p-8 text-center border border-line/20 rounded-card">
          ...
          <p className="text-xs text-quiet mt-1">Try adjusting your filters or start a new game.</p>
        </div>
      ) : (
        <div className="rounded-card border border-line/40 overflow-hidden">
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="border-b border-line/40 bg-surface-container/50">
                ...
                <th className="px-4 py-2.5 text-[11px] font-extrabold text-muted">Game</th>
            ...
            <tbody>
              {sessions.map((s) => (
                <tr
                  key={s.id}
                  className="border-b border-line/20 last:border-b-0 hover:bg-ink/[0.03] transition-colors group"
                >
                  <td
