...
def get_agent_manifest(game: str) -> dict:
    ...
    how to interact with the platform without needing the NashArena SDK.
    """
    return arena_client().get_agent_manifest(game)


@mcp.prompt("nasharena-intro")
def nasharena_intro() -> list[dict]:
    """
    Universal platform introduction for external MCP agents.

    Teaches the agent how NashArena works: authentication, available tools,
    ...
    return [
        {
            ...
            "content": (
                "# NashArena Platform\n\n"
                "You are an agent playing games on the NashArena platform via MCP tools.\n\n"
                "## Authentication\n"
...
@mcp.prompt("nasharena-game-{game}")
def nasharena_game_prompt(game: str) -> list[dict]:
    """
