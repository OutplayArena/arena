...
interface CentipedeConfigFormProps {
  gameSlug: string;
  locked: boolean;
...
export default function CentipedeConfigForm({ gameSlug, locked, sessionStatus, initialValues }: CentipedeConfigFormProps) {
  ...
  const [agentAId, setAgentAId] = useState("last_step_take");
  const [agentBId, setAgentBId] = useState("always_pass");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const maxSteps = maxStepsRef.current ? Number(maxStepsRef.current.value) : 6;
    ...
    const config = {
      ...
      interactive: true,
      max_steps: maxSteps,
      initial_pot_a: potARef.current ? Number(potARef.current.value) : 4.0,
      initial_pot_b: potBRef.current ? Number(potBRef.current.value) : 1.0,
      growth_factor: growthRef.current ? Number(growthRef.current.value) : 2.0,
    };
  ...
  return (
    <div>
      <form className="grid gap-3 p-4" onSubmit={handleSubmit}>
        ...
        <div className="px-3 py-2 rounded bg-rose-50 dark:bg-rose-950/20 border border-rose-200 dark:border-rose-800/40">
          <p className="text-[11px] text-rose-700 dark:text-rose-300 font-semibold">Players alternate PASS or TAKE. Each PASS doubles the pot; TAKE ends the game. Backward induction predicts immediate TAKE.</p>
        </div>

        <label className="grid gap-1 text-muted text-[11px] font-extrabold">
          <span>Agent A (moves first)</span>
        ...
        <label className="grid gap-1 text-muted text-[11px] font-extrabold">
          <span>max_steps (2–20)</span>
          <input ref={maxStepsRef} type="number" min={2} max={20} defaultValue={6} className={inputClass} disabled={formDisabled} />
        </label>

        <div className="grid gap-1">
          <span className="text-muted text-[11px] font-extrabold">Initial pots & growth</span>
          <div className="grid grid-cols-3 gap-2">
            <label className="grid gap-0.5">
              <span className="text-[10px] text-quiet font-semibold">Pot A (step 1)</span>
              <input ref={potARef} type="number" step="1" min={1} defaultValue={4} className={inputClass} disabled={formDisabled} />
            </label>
            <label className="grid gap-0.5">
              <span className="text-[10px] text-quiet font-semibold">Pot B (step 1)</span>
              <input ref={potBRef} type="number" step="1" min={1} defaultValue={1} className={inputClass} disabled={formDisabled} />
            </label>
            <label className="grid gap-0.5">
              <span className="text-[10px] text-quiet font-semibold">Growth factor</span>
              <input ref={growthRef} type="number" step="0.5" min={1.1} defaultValue={2} className={inputClass} disabled={formDisabled} />
            </label>
          </div>
        </div>
