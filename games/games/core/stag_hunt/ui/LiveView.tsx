...
const ACTION_EMOJI: Record<string, string> = { stag: "🦌", hare: "🐇" };
const ACTION_LABEL: Record<string, string> = { stag: "Stag", hare: "Hare" };
const ACTION_COLOR: Record<string, string> = {
  stag: "text-emerald-600 dark:text-emerald-400",
  hare: "text-amber-600 dark:text-amber-400",
};
const OUTCOME_STYLE: Record<string, string> = {
  SS: "bg-emerald-100 text-emerald-700 border-emerald-200 dark:bg-emerald-950/40 dark:text-emerald-300 dark:border-emerald-700/40",
  HH: "bg-amber-100 text-amber-700 border-amber-200 dark:bg-amber-950/40 dark:text-amber-300 dark:border-amber-700/40",
  SH: "bg-red-100 text-red-700 border-red-200 dark:bg-red-950/40 dark:text-red-300 dark:border-red-700/40",
  HS: "bg-red-100 text-red-700 border-red-200 dark:bg-red-950/40 dark:text-red-300 dark:border-red-700/40",
};

function agentInitial(name: string) { return (name.trim().charAt(0) || "?").toUpperCase(); }

function getAction(r: MatchRound, side: "A" | "B"): string | undefined {
  return (r.raw?.actions as Record<string, string> | undefined)?.[side];
}

function getOutcome(r: MatchRound): string | undefined {
  return r.raw?.outcome as string | undefined;
}

function outcomeCounts(history: MatchRound[]) {
  let ss = 0, hh = 0, sh = 0, hs = 0;
...
export default function StagHuntLiveView() {
  const { state } = useApp();
  ...
  const coordinationRate = history.length > 0
    ? Math.round(((counts.ss + counts.hh) / history.length) * 100)
    : 0;
        ...
        <div className="flex items-center justify-between px-4 py-2.5">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <div className="text-[10px] text-quiet font-mono tabular-nums">{totalScoreA.toFixed(1)} pts</div>
            </div>
          </div>

          <div className="flex flex-col items-center gap-0.5">
            <div className="flex items-center gap-2 text-xs font-mono">
              <span className="font-black text-emerald-600 dark:text-emerald-400">SS:{counts.ss}</span>
              <span className="text-quiet">·</span>
              <span className="font-black text-amber-600 dark:text-amber-400">HH:{counts.hh}</span>
              <span className="text-quiet">·</span>
              <span className="font-black text-red-600 dark:text-red-400">Miss:{counts.sh + counts.hs}</span>
            </div>
            <div className="text-[10px] text-quiet font-medium">
              {coordinationRate}% coordination
            </div>
            {matchComplete && (
          ...
          <div className="flex items-center gap-2 min-w-0">
            <div className="min-w-0 text-right">
              ...
              <div className="text-[10px] text-quiet font-mono tabular-nums">{totalScoreB.toFixed(1)} pts</div>
            </div>
            ...
            return (
              ...
              >
                ...
                <div className={`flex items-center gap-1.5 flex-1 ${actionA ? ACTION_COLOR[actionA] : "text-quiet"}`}>
                  <span className="text-base">{actionA ? ACTION_EMOJI[actionA] : "?"}</span>
                  <span className="text-xs font-bold">{actionA ? ACTION_LABEL[actionA] : "—"}</span>
                </div>
                ...
                <div className={`flex items-center gap-1.5 flex-1 justify-end ${actionB ? ACTION_COLOR[actionB] : "text-quiet"}`}>
                  <span className="text-xs font-bold">{actionB ? ACTION_LABEL[actionB] : "—"}</span>
                  <span className="text-base">{actionB ? ACTION_EMOJI[actionB] : "?"}</span>
                </div>
