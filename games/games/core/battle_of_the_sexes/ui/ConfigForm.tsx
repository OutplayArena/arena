...
export default function BoSConfigForm({ gameSlug, locked, sessionStatus, initialValues }: BoSConfigFormProps) {
  ...
  const [agentAId, setAgentAId] = useState("always_a");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const config = {
      ...
      payoff_preferred_b: prefBRef.current ? Number(prefBRef.current.value) : 3.0,
      payoff_nonpreferred: nonPrefRef.current ? Number(nonPrefRef.current.value) : 2.0,
      payoff_mismatch: 0.0,
    };
  ...
  return (
    <div>
      <form className="grid gap-3 p-4" onSubmit={handleSubmit}>
        ...
        <div className="px-3 py-2 rounded bg-violet-50 dark:bg-violet-950/20 border border-violet-200 dark:border-violet-800/40">
          <p className="text-[11px] text-violet-700 dark:text-violet-300 font-semibold">Agent A prefers option A · Agent B prefers option B · both prefer coordination over mismatch</p>
        </div>
        ...
        <div className="grid gap-1">
          <span className="text-muted text-[11px] font-extrabold">Payoffs</span>
          <div className="grid grid-cols-3 gap-2">
            <label className="grid gap-0.5">
              <span className="text-[10px] text-quiet font-semibold">A preferred</span>
              <input ref={prefARef} type="number" step="0.5" defaultValue={3.0} className={inputClass} disabled={formDisabled} />
            </label>
            <label className="grid gap-0.5">
              <span className="text-[10px] text-quiet font-semibold">B preferred</span>
              <input ref={prefBRef} type="number" step="0.5" defaultValue={3.0} className={inputClass} disabled={formDisabled} />
            </label>
            <label className="grid gap-0.5">
              <span className="text-[10px] text-quiet font-semibold">Non-preferred</span>
              <input ref={nonPrefRef} type="number" step="0.5" defaultValue={2.0} className={inputClass} disabled={formDisabled} />
            </label>
          </div>
        </div>
