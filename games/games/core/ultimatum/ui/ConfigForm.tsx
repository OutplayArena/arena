...
interface UltimatumConfigFormProps {
  gameSlug: string;
  locked: boolean;
...
export default function UltimatumConfigForm({ gameSlug, locked, sessionStatus, initialValues }: UltimatumConfigFormProps) {
  ...
  const [agentAId, setAgentAId] = useState("fair");
  ...
  return (
    <div>
      <form className="grid gap-3 p-4" onSubmit={handleSubmit}>
        ...
        <div className="px-3 py-2 rounded bg-sky-50 dark:bg-sky-950/20 border border-sky-200 dark:border-sky-800/40">
          <p className="text-[11px] text-sky-700 dark:text-sky-300 font-semibold">Roles alternate each round — both players act as proposer and responder equally</p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          <label className="grid gap-1 text-muted text-[11px] font-extrabold">
            <span>total (pot size)</span>
            <input ref={totalRef} type="number" step="10" min={1} defaultValue={100} className={inputClass} disabled={formDisabled} />
          </label>
          <label className="grid gap-1 text-muted text-[11px] font-extrabold">
            <span>min_offer</span>
