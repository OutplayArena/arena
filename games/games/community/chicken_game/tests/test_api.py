...
class TestCGConfig:
    def test_config_from_request_builds_cg_config(self):
        config = config_from_request(_valid_payload())
        assert config.game == "chicken_game"
        assert config.rounds == 3
        assert config.payoff_win == 1.0
        assert config.payoff_tie == 0.0
        assert config.payoff_lose == -1.0
        assert config.payoff_crash == -10.0

    def test_config_rejects_invalid_payoff_order(self):
        payload = _valid_payload(payoff_win=1.0, payoff_tie=2.0)
        with pytest.raises(ValueError, match="payoff_win > payoff_tie"):
...
class TestCGExperimentCreation:
    ...
    def test_create_rejects_invalid_payoffs(self, fake_db):
        client = TestClient(app)
        response = client.post("/experiment", json=_valid_payload(payoff_win=1.0, payoff_tie=2.0))
...
class TestCGState:
    ...
    def test_state_includes_payoff_structure(self, fake_db):
        ...
        assert state["payoffs"]["win"] == 1.0


class TestCGActionSubmission:
    ...
    def test_duplicate_action_returns_conflict(self, fake_db):
        ...
        response = client.post(
            f"/session/{created['session_id']}/action",
            headers={"Authorization": f"Bearer {token_a}"},
            json={"allocation": "swerve"},
        )
        assert response.status_code == 409


class TestCGResults:
    def test_mutual_swerve(self, fake_db):
        client = TestClient(app)
        created = client.post("/experiment", json=_valid_payload()).json()
...
class TestCGDirectoryEndpoints:
    def test_list_games_includes_chicken_game(self, fake_db):
        client = TestClient(app)
        slugs = [g["slug"] for g in client.get("/games").json()]
        assert "chicken_game" in slugs

    def test_game_details(self, fake_db):
        client = TestClient(app)
        response = client.get("/games/chicken_game")
        assert response.status_code == 200
        assert response.json()["name"] == "Chicken Game"

    def test_metrics_declaration(self, fake_db):
        client = TestClient(app)
        assert client.get("/games/chicken_game/metrics").status_code == 200

    def test_prompts(self, fake_db):
        client = TestClient(app)
        assert client.get("/games/chicken_game/prompts").status_code == 200
