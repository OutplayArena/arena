...
const ACTION_EMOJI: Record<string, string> = { dare: "🏎️", swerve: "🚗" };
const ACTION_LABEL: Record<string, string> = { dare: "Dare", swerve: "Swerve" };
const ACTION_COLOR: Record<string, string> = {
  dare: "text-red-600 dark:text-red-400",
  swerve: "text-emerald-600 dark:text-emerald-400",
};
const OUTCOME_STYLE: Record<string, string> = {
  ...
  DS: "bg-amber-100 text-amber-700 border-amber-200 dark:bg-amber-950/40 dark:text-amber-300 dark:border-amber-700/40",
  SD: "bg-amber-100 text-amber-700 border-amber-200 dark:bg-amber-950/40 dark:text-amber-300 dark:border-amber-700/40",
};
...
export default function ChickenGameLiveView() {
  const { state } = useApp();
  ...
  const crashRate = history.length > 0 ? Math.round((counts.dd / history.length) * 100) : 0;
        ...
        <div className="flex items-center justify-between px-4 py-2.5">
          ...
          <div className="flex flex-col items-center gap-0.5">
            <div className="flex items-center gap-2 text-xs font-mono">
              <span className="font-black text-red-600 dark:text-red-400">Crash:{counts.dd}</span>
              <span className="text-quiet">·</span>
              <span className="font-black text-emerald-600 dark:text-emerald-400">Yield:{counts.ss}</span>
              <span className="text-quiet">·</span>
              <span className="font-black text-amber-600 dark:text-amber-400">Win:{counts.ds + counts.sd}</span>
            </div>
