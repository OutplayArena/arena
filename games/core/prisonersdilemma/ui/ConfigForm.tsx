...
export default function PDConfigForm({ gameSlug, locked, sessionStatus, initialValues }: PDConfigFormProps) {
  ...
  const [agentAId, setAgentAId] = useState("tit_for_tat");
  const [agentBId, setAgentBId] = useState("always_defect");
  ...
  useEffect(() => {
    ...
    if (isReplay && initialValues) {
      ...
      if (initialValues.variant) setVariant(String(initialValues.variant));
      if (roundsRef.current && initialValues.rounds !== undefined) roundsRef.current.value = String(initialValues.rounds);
      if (noiseRef.current && initialValues.noise !== undefined) noiseRef.current.value = String(initialValues.noise);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const noise = variant === "noisy" && noiseRef.current ? Number(noiseRef.current.value) : 0.0;
      ...
      <form className="grid gap-3 p-4" onSubmit={handleSubmit}>
        ...
        <label className="grid gap-1 text-muted text-[11px] font-extrabold">
          ...
          <select value={variant} onChange={(e) => setVariant(e.target.value)} className={inputClass} disabled={formDisabled}>
            ...
            <option value="noisy">Noisy (random action flips)</option>
          </select>
        </label>

        {variant === "noisy" && (
          <label className="grid gap-1 text-muted text-[11px] font-extrabold">
            <span>noise (flip probability)</span>
            <input ref={noiseRef} type="number" min={0} max={0.5} step={0.01} defaultValue={0.05} className={inputClass} disabled={formDisabled} />
          </label>
        )}
