...
export default function CournotConfigForm({ gameSlug, locked, sessionStatus, initialValues }: CournotConfigFormProps) {
  ...
  const [agentAId, setAgentAId] = useState("collusive");
  const [agentBId, setAgentBId] = useState("nash");
    ...
    <div>
      <form className="grid gap-3 p-4" onSubmit={handleSubmit}>
        ...
        <label className="grid gap-1 text-muted text-[11px] font-extrabold">
          ...
          <input type="text" value={agentAName} onChange={(e) => setAgentAName(e.target.value)} className={inputClass} disabled={formDisabled} placeholder="Firm display name" />
        </label>
        ...
        <label className="grid gap-1 text-muted text-[11px] font-extrabold">
          ...
          <input type="text" value={agentBName} onChange={(e) => setAgentBName(e.target.value)} className={inputClass} disabled={formDisabled} placeholder="Firm display name" />
        </label>
          ...
          {Object.entries(sessionKeys).map(([player, key]) => (
            <div key={player} className="mb-2 last:mb-0">
              <div className="flex items-center gap-2">
                <span className="text-[11px] font-extrabold text-muted uppercase">Firm {player}</span>
