#!/usr/bin/env python3
"""
Docker E2E Test for MCP Gateway Architecture

This script tests the complete MCP gateway flow in Docker:
...
BACKEND_URL = f"http://{_backend_ip}:8000/api"
API_KEY = ""
JWT_SECRET = ""
...
async def main():
    print("=" * 70)
    print("Docker E2E Test: MCP Gateway Architecture")
    print("=" * 70)
    print()
    ...
    # Step 6: Verify container cleanup
    print("Step 6: Verifying container cleanup...")
    import subprocess
    result = subprocess.run(
        ["docker", "ps", "-q", "--filter", "label=mcp-instance"],
        capture_output=True,
        text=True,
    )
    containers = result.stdout.strip()
    if containers:
        print(f"  WARNING: MCP containers still running: {containers}")
    else:
