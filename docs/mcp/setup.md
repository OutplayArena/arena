...
Configuration and deployment of MCP servers for NashArena.

## Prerequisites

- NashArena backend running
...
```bash
# Check MCP pods
kubectl -n nasharena get pods | grep mcp

# Check RBAC
kubectl -n nasharena get rolebindings
...
# View logs
kubectl -n nasharena logs mcp-abc123

# Check resource usage
kubectl -n nasharena top pods -l app=mcp-server
```
