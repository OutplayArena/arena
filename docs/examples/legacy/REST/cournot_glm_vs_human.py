"""
Cournot Duopoly: GLM-5.1 vs Human Player

This agent joins an existing Cournot Duopoly game session using a session key
and plays as GLM-5.1 against a human player.

Usage:
    export SESSION_KEY="nks_..."
    export OPENCODE_GO_API_KEY="..."
    python cournot_glm_vs_human.py
"""
...
def parse_quantity(text, max_qty):
...
async def play_game():
    ...
    print(f"=== Cournot Duopoly: {MODEL} vs Human ===")
    ...
    max_quantity = state.get("max_quantity", 120)
    total_rounds = state.get("round_total", 10)
    demand_a = state.get("demand_a", 120)
    demand_b = state.get("demand_b", 1)
    cost = state.get("cost_per_unit", 0)
    nash_qty = (demand_a - cost) / (3 * demand_b)
    collusive_qty = (demand_a - cost) / (4 * demand_b)

    print(f"Market: demand_a={demand_a}, demand_b={demand_b}, cost={cost}, max_qty={max_quantity}")
    print(f"Nash equilibrium: {nash_qty:.1f} units")
    print(f"Collusive quantity: {collusive_qty:.1f} units")
    ...
    while True:
        ...
        print(f"  Profits: A={total_scores.get('A', 0):.1f} B={total_scores.get('B', 0):.1f}")
        ...
        if error:
            qty = nash_qty
            print(f"  {MODEL} error - using Nash fallback: {qty:.1f}")
        else:
            qty = parse_quantity(raw_response, max_quantity)
        ...
        print(f"  Submitted: {qty:.1f} units")
        print()
    ...
    print(f"Final Profits: A={total_a:.1f}  B={total_b:.1f}")
    print()
    ...
    if history:
        ...
        for h in history:
            r = h.get("round", "?")
            a_qty = h.get("quantities", {}).get("A", 0)
            b_qty = h.get("quantities", {}).get("B", 0)
            price = h.get("price", 0)
            a_profit = h.get("payoffs", {}).get("A", 0)
            b_profit = h.get("payoffs", {}).get("B", 0)
            print(f"  R{r}: A={a_qty:.1f} B={b_qty:.1f} -> Price={price:.1f} | A={a_profit:.1f} B={b_profit:.1f}")

    os.makedirs(RESULTS_DIR, exist_ok=True)
    timestamp = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
    result_file = os.path.join(RESULTS_DIR, f"cournot_{MODEL}_vs_human_{timestamp}.json")
